...
function shorterestTime(n,m,[singleFloorEl, doorsOpen, doorsClose, singleFloorWalk]) {
    const walkingTime = n * singleFloorWalk
    const timeToElevator = (start, end) => (start > end ? start - end : end - start) * singleFloorEl
    const elevatorAllToGround = (doorsOpen + doorsClose) + (n * singleFloorEl) + doorsOpen
    const elevatorToGround = floorLevel => (doorsOpen + doorsClose) + (floorLevel * singleFloorEl) + doorsOpen
    const elevatorTotal = timeToElevator(m, n) + elevatorToGround(n)
    const possibleWalkingCombinations = []
    for(let floor = 0; floor < (n - m + 1); floor++){
      const time = (singleFloorWalk * floor) + timeToElevator(n-floor, m) + elevatorToGround(n-floor)
      possibleWalkingCombinations.push(time)
    }
    const times = [walkingTime, elevatorTotal, ...possibleWalkingCombinations]
...
function shorterestTime(n, m, [a, b, c, d]) {
    const feetTime = n * d;
    const elevTime = Math.abs(n - m) * a + n * a + 2 * b + c;
    const bothTime = Math.abs(n - m) * d + m * a + 2 * b + c;
    
    return Math.min(feetTime, elevTime, bothTime);
  }